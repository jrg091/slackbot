...
import com.xmartlabs.slackbot.model.BambooHrDirectoryUser
...
object BambooHrReportsRemoteSource {
    ...
    suspend fun getUsers(): List<BambooUser> = getEmployees()
        .mapNotNull { directoryUser -> generateBambooUser(directoryUser) }
    ...
    suspend fun getUser(email: String) = getEmployees()
        .find { it.workEmail == email }
        ?.let { generateBambooUser(it) }

    private suspend fun generateBambooUser(directoryUser: BambooHrDirectoryUser): BambooUser? {
        ...
        return if (directoryUser.workEmail == null ||
            workingHours == null ||
